...
export const DriversView: React.FC<DriversViewProps> = ({ onSelectDriverHistory }) => {
  ...
  const [commissionRate, setCommissionRate] = useState<number | string>(2.5);
  ...
  const handleOpenModal = (driver?: Driver) => {
    if (driver) {
      ...
      setCommissionRate(driver.commission_rate_default || 2.5);
  ...
  const handleSaveDriver = (e: React.FormEvent) => {
    ...
    storage.saveDriver({
      ...
      commission_rate_default: Number(commissionRate) || 2.5,
      status,
      observation,
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500">
            Cadastre a equipe de entregas, comissões individuais e acompanhe o desempenho.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {drivers.map((driver) => {
          ...
          const totalCommission = driverSales.reduce((acc, curr) => acc + curr.commission_amount, 0);
              ...
              <div>
                ...
                <div className="space-y-1.5 text-xs text-slate-600 border-t border-slate-100 pt-3">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-bold text-sky-700">{driver.commission_rate_default}%</span>
                  </div>
                </div>
                ...
                <div className="mt-4 p-3 bg-slate-50 rounded-xl border border-slate-100 grid grid-cols-3 gap-2 text-center">
                  ...
                  <div>
                    <span className="text-[10px] text-slate-400 block font-medium">Comissão</span>
                    <span className="text-xs font-black text-indigo-700">{formatCurrency(totalCommission)}</span>
                  </div>
                </div>
              </div>
